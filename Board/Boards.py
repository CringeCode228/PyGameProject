...
class Board:
    ...
    def get_click(self, mouse_pos, key):
        cell = self.get_cell(mouse_pos)
        if cell:
            self.on_click(self.get_cell(mouse_pos), key)
...
class GameField(Board):
    ...
    def on_click(self, cell_coords, key):
        if key == 1:
            try:
                self.objects.remove(self.board[cell_coords[0]][cell_coords[1]])
                self.board[cell_coords[0]][cell_coords[1]] = 0
            except Exception as e:
                print(type(e))
                pass
            self.board[cell_coords[0]][cell_coords[1]] = \
                self.inventory.board[0][self.inventory.active_cell].clone(cell_coords[0], cell_coords[1], self)
...
class GameFieldInventory(Board):
    ...
    def render(self):
        ...
        # pygame.draw.line(self.screen, pygame.Color(255, 255, 255), (self.left, self.top),
        #                  (self.left + self.height * self.cell_size, self.top))
        # pygame.draw.line(self.screen, pygame.Color(255, 255, 255), (self.left, self.top + self.cell_size),
        #                  (self.left + self.height * self.cell_size, self.top + self.cell_size))
        # for i in range(self.height + 1):
        #     pygame.draw.line(self.screen, pygame.Color(255, 255, 255), (self.left + self.cell_size * i, self.top),
        #                      (self.left + self.cell_size * i, self.top + self.cell_size))
        self.objects.draw(self.screen)
        # for i in range(self.width):
